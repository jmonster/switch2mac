...
@main enum PolicyTests {
    static func main() {
        ...
        queue.sync {
            ...
            precondition(policy.policy.work == nil)
            defaults.set(true, forKey: DiscoveryPolicy.enabledKey)
            precondition(policy.shouldScan(readyIDs: [a], now: 0))
            let firstDeadline = policy.policy.work!
            precondition(policy.shouldScan(readyIDs: [a, b], now: 30))
            precondition(policy.policy.work === firstDeadline, "unrelated state must not extend the setup window")
            ...
            let stale = policy.policy.work!
            policy.openWindow(now: 63)
            stale.perform()
            precondition(changes.value.withLock { $0 } == 0 && policy.windowIsOpen(now: 70))
            policy.policy.work!.perform()
            precondition(changes.value.withLock { $0 } == 1 && policy.policy.work == nil)
            ...
            precondition(policy.shouldScan(readyIDs: [a], now: 103) && policy.policy.work == nil)
