...
swiftc -swift-version 6 -warnings-as-errors Sources/GameCubed/Protocol/Switch2Protocol.swift \
 Sources/GameCubed/Runtime/SensorProfileReport.swift tests/power-profiles/ReportTests.swift -o "$work/report"
...
python3 - <<'PYCODE'
from pathlib import Path
entry = Path('Sources/GameCubed/Runtime/ApplicationEntry.swift').read_text()
assert entry.index('contains("--sensor-profile")') < entry.index('GameCubedApp.main()')
